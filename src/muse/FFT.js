...
export default class FFT {
    ...
    forward(buffer) {
        ...
        var hammingWindow = new Float64Array(bufferSize);
        ...
        // Apply the window function here before the FFT
        for (i = 0; i < bufferSize; i++) {
            real[i] = buffer[reverseTable[i]]// * hammingWindow[i];  // Apply Hamming window
            imag[i] = 0;
        }
        ...
        while (halfSize < bufferSize) {
         ...
         // Updating PSD and converting to dB
        for (var ii = 0, N = bufferSize / 2; ii < N; ii++) {
            //let magSquared = real[ii] * real[ii] + imag[ii] * imag[ii];
            // Convert magnitude squared to dB; added a small constant to avoid log(0)
            //spectrum[ii] = 10 * Math.log10(magSquared + 1e-6);

            //matches mind control version with accurate ML
            let mag = Math.sqrt(real[ii] * real[ii] + imag[ii] * imag[ii]);
            spectrum[ii] = mag;
        }
