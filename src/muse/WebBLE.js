...
export default class WebBLE {
  ...
  connect(serviceUuidOrOptions, onConnectedCallback, onDisconnectedCallback) {
    ...
    return callCallback(navigator.bluetooth.requestDevice(options)
      .then((device) => {
        ...
        // Set up the disconnection event listener
      ...
      .then((characteristics) => {
        ...
        // Call the onConnectedCallback here, after successful connection
        if (typeof onConnectedCallback === 'function') {
          onConnectedCallback(characteristics);
        }
      })
      .catch((error) => {
